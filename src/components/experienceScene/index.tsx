...
const HeroContent = () => (
  <div className="mx-auto max-w-3xl text-center">
    <p className="exp-reveal font-mono text-sm uppercase tracking-[0.4em] text-white/60" style={{ ['--i' as string]: 0 }}>
      The Journey
    </p>
    <div className="mt-4">
      <h1
        className="exp-wipe pb-2 text-5xl font-black leading-none text-white sm:text-7xl md:text-8xl"
        style={{ ['--i' as string]: 1 }}
      >
        EXPERIENCE
      </h1>
    ...
    <p
      className="exp-reveal mx-auto mt-6 max-w-xl text-lg leading-relaxed text-white/80"
      style={{ ['--i' as string]: 2 }}
    >
...
const OutroContent = () => (
  <div className="mx-auto max-w-2xl text-center">
    <div className="pb-1">
      <h2 className="exp-wipe pb-1 text-4xl font-black leading-tight text-white sm:text-6xl" style={{ ['--i' as string]: 0 }}>
        The next chapter?
      </h2>
    </div>
    <p className="exp-reveal mx-auto mt-6 max-w-lg text-lg leading-relaxed text-white/80" style={{ ['--i' as string]: 1 }}>
    ...
    <div className="exp-reveal mt-9 flex flex-wrap items-center justify-center gap-4" style={{ ['--i' as string]: 2 }}>
      ...
      <Link
        to="/Portfolio"
        className="cursor-none rounded-full border px-7 py-3 text-base font-bold text-white transition-colors hover:bg-white/10"
    ...
    <div
      className="exp-reveal mt-10 flex items-center justify-center gap-6 text-4xl text-white"
      style={{ ['--i' as string]: 3 }}
    >
      <LinkedinIcon />
...
const scenes: Shell[] = [
  { key: 'hero', label: 'Start', theme: heroTheme, Art: HeroArt, divider: dividers[0], scrim: 'none', content: <HeroContent /> },
  ...roles.map((role, i) => ({
    key: role.id,
    label: roleLabels[i],
    theme: role.theme,
  ...
  { key: 'outro', label: 'Connect', theme: outroTheme, Art: OutroArt, divider: dividers[7], scrim: 'none', content: <OutroContent /> },
]
    ...
    <div className="exp-journey">
      ...
      <nav className="exp-rail" aria-label="Experience chapters">
        {scenes.map((s, i) => (
          <button
            ...
            data-mousecustom={linkMouse}
            style={{ ['--rail-accent' as string]: s.theme.accent } as React.CSSProperties}
          >
      ...
      {scenes.map((s, i) => (
        <SceneShell
          ...
          active={active === i}
          divider={s.divider}
          dividerColor={scenes[i + 1]?.theme.bgFrom ?? 'transparent'}
          scrim={s.scrim}
          label={s.label}
        >
          {s.content}
